...
class VIPAdminPanel {
    ...
    setupPermissions() {
        ...
        const userType = this.currentUser.accountType;
        const permissions = this.config.HIERARCHY.PERMISSIONS[userType] || [];
        ...
        if (userType === 'god' || userType === 'admin') {
            document.getElementById('creditsBadge').style.display = 'none';
        } else {
            document.getElementById('creditsBadge').style.display = 'block';
        ...
        if (permissions.includes('create_all')) {
            deviceTypeEl.innerHTML = `
                <option value="single">Single Device</option>
                <option value="double">Double Device</option>
        ...
        accountTypeEl.innerHTML = options;
    ...
    updateFormVisibility() {
        ...
        const showCredits = (accountType === 'reseller' || accountType === 'seller') &&
            (this.currentUser.accountType !== 'user' && this.currentUser.accountType !== 'reseller');
        creditsGroup.style.display = showCredits ? 'block' : 'none';
    }

    updateCreateButtonText() {
        const createBtn = document.getElementById('createUserBtn');
        if (!createBtn || !this.currentUser) return;
        const accountType = document.getElementById('accountType').value;

        if (this.currentUser.accountType === 'god' || this.currentUser.accountType === 'admin') {
            createBtn.textContent = `Create ${accountType}`;
            return;
        }

        if (accountType === 'reseller' || accountType === 'seller') {
            const creditsToGive = document.getElementById('creditsToGive').value || '0';
            createBtn.textContent = `Create ${accountType} (-${creditsToGive} Credits)`;
        } else {
            const expiryPeriod = document.getElementById('expiryPeriod').value;
            const deviceType = document.getElementById('deviceType').value;
            const cost = this.calculateCreditCost(expiryPeriod, deviceType);
            createBtn.textContent = `Create User (-${cost} Credits)`;
        }
    }
    ...
    calculateCreditCost(expiryPeriod, deviceType) {
        ...
        const multiplier = this.config.CREDITS.DEVICE_MULTIPLIER[deviceType] || 1;
    ...
    async handleCreateUser(e) {
        ...
        const userData = {
            ...
            expiryPeriod: document.getElementById('expiryPeriod').value,
            deviceType: document.getElementById('deviceType').value,
            accountType: document.getElementById('accountType').value,
            creditsToGive: parseInt(document.getElementById('creditsToGive').value) || 0,
        };
    ...
    async createUser(userData) {
        let creditCost = 0;
        if (this.currentUser.accountType !== 'god' && this.currentUser.accountType !== 'admin') {
            creditCost = (userData.accountType === 'reseller' || userData.accountType === 'seller') ?
                userData.creditsToGive :
                this.calculateCreditCost(userData.expiryPeriod, userData.deviceType);
            if (this.currentUser.credits < creditCost) {
                throw new Error('Insufficient credits.');
            }
        }

        let expiryTimestamp;
        const period = parseFloat(userData.expiryPeriod);
        if (period === 9999) {
            expiryTimestamp = '9999';
        } else {
            const seconds = Math.floor(period * 3600);
            expiryTimestamp = Math.floor(Date.now() / 1000) + seconds;
        }
        ...
        const newUser = {
            records: [{
                fields: {
                    ...
                    Credits: (userData.accountType === 'reseller' || userData.accountType === 'seller') ? userData.creditsToGive : 0,
        ...
        if (creditCost > 0) {
            ...
            await this.updateUserCredits(this.currentUser.recordId, newCreditTotal);
            this.currentUser.credits = newCreditTotal;
            document.getElementById('userCredits').textContent = newCreditTotal;
        }
    }
    ...
    async loadUsers() {
        ...
        try {
            ...
            if (this.currentUser.accountType === 'admin') {
                url += `?filterByFormula=NOT({AccountType}='god')`;
    ...
    renderUsersTable() {
        ...
        this.allUsers.forEach(record => {
            ...
            row.innerHTML = `
                ...
                <td>${(user.AccountType === 'seller' || user.AccountType === 'reseller') ? user.Credits || 0 : '-'}</td>
                ...
                <td class="action-buttons">
                    <button onclick="app.deleteUser('${record.id}', '${user.Username}')" class="action-btn btn-danger">Delete</button>
                </td>
            `;
    ...
    updateStats() {
        ...
        document.getElementById('resellerCount').textContent = this.allUsers.filter(r => r.fields.AccountType === 'reseller').length;
    }
